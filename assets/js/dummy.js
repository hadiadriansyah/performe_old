(function($) {
    ...
    function customizeDataTableUI() {
        $('#tablePerspective').each(function() {
            ...
            searchInput.attr('placeholder', 'Search').removeClass('form-control-sm');
            const lengthSelect = datatable.closest('#tablePerspective_wrapper').find('div[class="dt-length"] select');
            lengthSelect.removeClass('form-control-sm');
        });
    }
    ...
    function initializeSelect2() {
        ...
        $('#yearPeriodId').each(function () {
            $(this).select2({
                ...
                dropdownParent: $(this).parent(),
                ajax: {
                    url: `${config.siteUrl}master/perspective/get_year_periods`,
                    dataType: 'json',
                    delay: 250,
                    data: params => ({
                        q: params.term || '',
    ...
    async function fetchPerspectiveData(id) {
        try {
            const response = await fetch(`${config.siteUrl}master/perspective/get/${id}`, { method: 'GET' });
            const result = await response.json();
