(function($) {
    ...
    $('#formPolarization').on('reset', function() {
        $('#formPolarization').find('.error-message').html('');
        $('#formPolarization').find('.select2-js-basic').val(null).trigger('change');
        const newOption = new Option('- Choose -', '', true, true);
        $('#formPolarization').find('.select2-js-server').append(newOption).trigger('change');
    });
