(function($) {
    ...
    $('#formIndexScore').on('reset', function() {
        $('#formIndexScore').find('.error-message').html('');
        $('#formIndexScore').find('.select2-js-basic').val(null).trigger('change');
        const newOption = new Option('- Choose -', '', true, true);
        $('#formIndexScore').find('.select2-js-server').append(newOption).trigger('change');
    });
