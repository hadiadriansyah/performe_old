(function($) {
    ...
    function initializeSelect2() {
        ...
        $('#yearPeriodId').each(function () {
            $(this).select2({
                ...
                dropdownParent: $(this).parent(),
                ajax: {
                    url: `${config.siteUrl}performance_appraisal/unit_performance_appraisal/get_year_period_options`,
                    dataType: 'json',
                    delay: 250,
                    data: params => ({
                        q: params.term || '',
        ...
        $('#unitId').each(async function () {
            $(this).select2({
                ...
                dropdownParent: $(this).parent(),
                ajax: {
                    url: `${config.siteUrl}performance_appraisal/unit_performance_appraisal/get_unit_options`,
                    dataType: 'json',
                    delay: 250,
                    data: params => ({
                        q: params.term || '',
    ...
    async function setupFormSubmission() {
        ...
        const formDetails = {
            url: 'performance_appraisal/unit_performance_appraisal/mapping_data',
            formData: formData
        };
    ...
    function handleFormResponse(response) {
        if (response.status === 'success') {
            ...
            setTimeout(() => {
                window.location.href = `${config.siteUrl}performance_appraisal/unit_performance_appraisal/kpi?data=${response.data.encrypted_data}`;
            }, 1000);
        } else {
