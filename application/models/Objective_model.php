...
class Objective_model extends CI_Model {
    protected $table = 'npm_objectives';
    protected $column_order = array(null, 'objective', null, 'year_period', 'created_at');
    protected $column_search = array('objective');
    protected $order = array('year_period' => 'desc', 'objective' => 'asc');
    ...
    public function exists(array $data) {
        $this->db->select("{$this->table}.*, npm_year_periods.year_period")
                    ->from($this->table)
                    ->join('npm_year_periods', "{$this->table}.year_period_id = npm_year_periods.id", 'left')
                    ->where("LOWER({$this->table}.objective)", strtolower($data['objective']))
    ...
    public function get_options_by_year_period_id($search, $page, $year_period_id) {
        ...
        $this->db->like("LOWER(CAST(objective AS TEXT))", $search);
        $this->db->where('year_period_id', $year_period_id);
        $this->db->limit(10, ($page - 1) * 10);
        $query = $this->db->get($this->table);

        $total_count = $this->db->like("LOWER(CAST(objective AS TEXT))", $search)->count_all_results($this->table);
