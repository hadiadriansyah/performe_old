...
class Directorate_model extends CI_Model {
    protected $table = 'npm_directorates';
    protected $column_order = array(null, 'directorate', null, 'year_period', 'created_at');
    protected $column_search = array('directorate');
    protected $order = array('year_period' => 'desc', 'directorate' => 'asc');
    ...
    public function exists(array $data) {
        $this->db->select("{$this->table}.*, npm_year_periods.year_period")
                    ->from($this->table)
                    ->join('npm_year_periods', "{$this->table}.year_period_id = npm_year_periods.id", 'left')
                    ->where("LOWER({$this->table}.directorate)", strtolower($data['directorate']))
    ...
    public function get_options_by_year_period_id($search, $page, $year_period_id) {
        ...
        $this->db->like("LOWER(CAST(directorate AS TEXT))", $search);
        $this->db->where('year_period_id', $year_period_id);
        $this->db->limit(10, ($page - 1) * 10);
        $query = $this->db->get($this->table);

        $total_count = $this->db->like("LOWER(CAST(directorate AS TEXT))", $search)->count_all_results($this->table);
