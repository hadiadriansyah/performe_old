...
class Perspective extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->repository('master/Perspective_repository');
        $this->repository = new Perspective_repository();
    ...
    public function index() {
        $data = array_merge($this->bc->get_global(), [
            'title' => 'Perspectives',
            'js' => [
                'admin/master/perspective.js'
            ],
        ]);
        $this->template->load('admin', 'admin/master/perspective/index', $data);
    }
    ...
    public function store() {
        ...
        if ($exists['is_exists']) {
            $this->json_response->error(
                'Data for the ' . $exists['data']->perspective . 
                ' perspective and the ' . $exists['data']->year_period . 
    ...
    private function validate_input() {
        $this->form_validation->set_rules('perspective', 'Perspective', 'required|trim|max_length[25]');
    ...
    private function collect_form_errors() {
        return [
            'perspective' => form_error('perspective'),
