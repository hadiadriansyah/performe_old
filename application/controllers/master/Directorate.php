...
class Directorate extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->repository('master/Directorate_repository');
        $this->repository = new Directorate_repository();
    ...
    public function index() {
        $data = array_merge($this->bc->get_global(), [
            'title' => 'Directorates',
            'js' => [
                'admin/master/directorate.js'
            ],
        ]);
        $this->template->load('admin', 'admin/master/directorate/index', $data);
    }
    ...
    public function store() {
        ...
        if ($exists['is_exists']) {
            $this->json_response->error(
                'Data for the ' . $exists['data']->directorate . 
                ' directorate and the ' . $exists['data']->year_period . 
    ...
    private function validate_input() {
        $this->form_validation->set_rules('directorate', 'Directorate', 'required|trim|max_length[25]');
    ...
    private function collect_form_errors() {
        return [
            'directorate' => form_error('directorate'),
