...
class Objective extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->repository('master/Objective_repository');
        $this->repository = new Objective_repository();
    ...
    public function index() {
        $data = array_merge($this->bc->get_global(), [
            'title' => 'Objectives',
            'js' => [
                'admin/master/objective.js'
            ],
        ]);
        $this->template->load('admin', 'admin/master/objective/index', $data);
    }
    ...
    public function store() {
        ...
        if ($exists['is_exists']) {
            $this->json_response->error(
                'Data for the ' . $exists['data']->objective . 
                ' objective and the ' . $exists['data']->year_period . 
    ...
    private function validate_input() {
        $this->form_validation->set_rules('objective', 'Objective', 'required|trim|max_length[75]');
